...
class RaidCog(commands.Cog):
        ...
        for guild in self.bot.guilds:
            ...
            if now > raid.spawned_at + duration_sec:
                ...
                if channel:
                    embed = discord.Embed(
                        title="⌛ RAID EXPIRED",
                        description=f"The time has run out! The boss has vanished, leaving the village in disarray.",
    ...
    async def raid(self, ctx):
        ...
        # Time remaining
        remaining_sec = (raid.spawned_at + (self.bot.raid_service.config["raid_duration_hours"] * 3600)) - int(time.time())
        if remaining_sec > 0:
