...
from database.db import RaidContribution as ContributionModel
...
class RaidService:
    def __init__(
        self, mobs: Dict[str, Dict], bosses: Dict[str, Dict], items: Dict[str, Dict], artifacts: Dict[str, Dict] = None
    ):
        self.mobs = mobs
    ...
    def calculate_rewards(self, session_factory, guild_id: int) -> List[Dict[str, Any]]:
        ...
        with session_factory() as session:
            last_raid = (
                session.query(RaidModel)
                .filter_by(guild_id=guild_id, is_active=False)
                .order_by(RaidModel.ended_at.desc())
                .first()
            )
            if not last_raid:
                return []
            ...
            contributions = (
                ...
                .filter_by(guild_id=guild_id, spawned_at=last_raid.spawned_at, is_claimed=False)
                .all()
            )
            ...
            # If the raid failed before completing Phase 1, mark claimed but don't give loot
            if last_raid.current_phase <= 1:
            ...
            for idx, cont in enumerate(sorted_cont):
                ...
                cont.is_claimed = True
                results.append({"user_id": user_id, "tier": tier, "loot": loot, "power": cont.total_power_donated})
    ...
    def check_spawn_trigger(self, session_factory, guild_id: int, channel_id: Optional[int] = None) -> bool:
        ...
        if guild_sum >= 0:
            self.spawn_raid(session_factory, guild_id, channel_id=channel_id)
            return True
        return False

    def spawn_raid(
        self, session_factory, guild_id: int, boss_id: Optional[str] = None, channel_id: Optional[int] = None
    ) -> RaidModel:
    ...
    def get_user_contribution(
        self, session_factory, guild_id: int, user_id: int, spawned_at: int
    ) -> Optional[ContributionModel]:
        """Get a user's contribution for a specific raid."""
        with session_factory() as session:
            return (
                session.query(ContributionModel)
                .filter_by(guild_id=guild_id, user_id=user_id, spawned_at=spawned_at)
                .first()
            )
    ...
    def donate_mob(self, session_factory, guild_id: int, user_id: int, mob_id: str, amount: int) -> Dict[str, Any]:
        ...
        with session_factory() as session:
            ...
            contribution = self._ensure_contribution_entry(
                session_factory, guild_id, user_id, raid.spawned_at, session=session
            )
            ...
            raid.current_power += power_donated
            contribution.total_power_donated += power_donated
            contribution.mobs_donated_this_phase += amount

            session.commit()

            phase_completed = False
            raid_finished = False
            if raid.current_power >= raid.target_power:
    ...
    def _advance_phase(self, session_factory, guild_id: int):
        ...
        with session_factory() as session:
            ...
            raid.target_power = self._calculate_target_power(
                session_factory, guild_id, next_phase["target_power_multiplier"]
            )
            raid.target_tag = None

            session.query(ContributionModel).filter_by(guild_id=guild_id, spawned_at=raid.spawned_at).update(
    ...
    def _ensure_contribution_entry(
        self, session_factory, guild_id: int, user_id: int, spawned_at: int, session=None
    ) -> ContributionModel:
        ...
        if session is not None:
            ...
            if not cont:
                cont = ContributionModel(guild_id=guild_id, user_id=user_id, spawned_at=spawned_at)
                session.add(cont)
                session.flush()
            return cont
        else:
